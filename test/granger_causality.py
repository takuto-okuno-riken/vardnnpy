...
from measures.pairwise_granger_causality import PairwiseGrangerCausality
from measures.multivaliate_granger_causality import MultivariateGrangerCausality
...
class TestGrangerCausality(object):
    ...
    def test(self):
        ...
        pgc = PairwiseGrangerCausality()
        pgc.plot(x=x)
        mgc = MultivariateGrangerCausality()
        mgc.plot(x=x)

        # set ex_signal time lag 1->3
        ex = si[node_num:node_num+2, 0:sig_len]
        x[2, 1:sig_len] = ex[0, 0:sig_len-1]
        pgc.plot(x=x, ex_signal=ex, is_full_node=1)
        mgc.plot(x=x, ex_signal=ex, is_full_node=1)
        ...
        ex_control[2, 1] = 0
        pgc.plot(x=x, ex_signal=ex, node_control=node_control, ex_control=ex_control, is_full_node=1)
        mgc.plot(x=x, ex_signal=ex, node_control=node_control, ex_control=ex_control, is_full_node=1)


if __name__ == '__main__':
    print('start granger causality test')
    start_time = datetime.datetime.now()
    test_gc = TestGrangerCausality()
    test_gc.test()
